...
def process_results_for_stats(input_file, output_file, df_columns, labels):
...
labels = ['LIM', 'COD', 'CAR', 'PAL', 'FAM', 'CIM']
df_columns = ['filename'] + labels + [label+':machine' for label in labels]

directory = '../temp/011718/'

epoch_num = 22
best_epoch_num = '017'
output_dir = 'CIM_results/'
set_type = 'train'
raw_file = directory + output_dir + best_epoch_num +'_'+ set_type+'.txt'
review_file = directory + output_dir + best_epoch_num +'_'+ set_type+'.csv'
note_level_output = directory  + output_dir + best_epoch_num + '_'+set_type+'_processed.csv'
note_level_stats = directory + output_dir + best_epoch_num+ '_'+set_type+'_stats.csv'

convert_output_to_dataframe(raw_file, review_file)
process_results_for_stats(review_file, note_level_output, df_columns, labels)
calc_stats(note_level_output, note_level_stats, labels)