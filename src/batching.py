...
def subset_notes(notes_file, annotations_file, output_directory, ratios):
	...
	for op in operators:
		...
		all_ids += list(operator_note_dict[op])
	...
	for person, ids in assign_note_dict.iteritems():
		...
		subset_df = notes_df[notes_df['ROW_ID'].isin(ids)]
		subset_df.to_csv(output_directory + person + '_EOL_Notes.csv')
	for x, y in assign_note_dict.iteritems():
		print x, len(y)
...
#concat_all_annotations_add_operators(directory + 'raw_data/cleaned_annotations/', directory + "raw_data/operators.csv", directory + "raw_data/all_annotations/op_annotations_112217.csv", text_columns, annotation_headers)
subset_notes(directory + "raw_data/all_notes/all_notes_cleaned.csv", directory + "raw_data/all_annotations/op_annotations_112217.csv", directory + "batches/", ratios)
